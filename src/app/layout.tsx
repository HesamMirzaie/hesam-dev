...
export default function RootLayout({ children }: LayoutProps<"/">) {
  return (
    <html lang="en" suppressHydrationWarning className={`${geistSans.variable} ${geistMono.variable} h-full antialiased`}>
      <head><Script id="theme-script" strategy="beforeInteractive">{themeScript}</Script></head>
            ...
            <div className="flex min-h-dvh flex-col pt-16">
              ...
              <footer className="mt-auto border-t border-border/80 bg-background/70 backdrop-blur">
                <div className="mx-auto grid w-full max-w-7xl gap-8 px-6 py-10 sm:grid-cols-2 sm:px-8 lg:px-10">
                  <div>
                    <p className="font-mono text-xs uppercase tracking-[0.2em] text-primary">HM / Portfolio</p>
                    <p className="mt-3 max-w-sm text-sm leading-6 text-muted-foreground">Scalable frontend systems, practical architecture, and product-minded engineering.</p>
                  </div>
                  <div className="flex items-end justify-between gap-4 sm:justify-end">
                    <a href={`mailto:${siteConfig.email}`} className="font-mono text-xs uppercase tracking-[0.16em] text-accent hover:underline">Start a conversation</a>
